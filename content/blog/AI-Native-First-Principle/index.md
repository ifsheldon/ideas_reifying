...
一开始我也是这么想的。首先，我们怎么知道用户想和 LLM 对话？算了，我做的还是 POC，就先假设用户会按下按钮和 LLM 对话吧，毕竟 ChatGPT 也差不多是这样做的🐶
...
而我们这个 LLM 语音对话系统里的（ASR + LLM）部分，跟（CV 模型 + GPT-3.5）的组合是惊人地相似。

> 咒术回战的粉丝可以想一下，假如与幸吉操纵机械的方式不是通过天赋直接和机器相连，而是通过发消息，会是什么样子 😂
>
...
所以这个例子的意义是什么呢？我觉得还是留给你们读者来总结。在这里，你们可以停一下，喝口咖啡，洗个澡，或者睡个觉，随便你。*Just let that sink in.*

![let_that_sink_in](./sink_in.png)

因为，下面我们要进入一个副本章节，一个令人激动的问题——OpenAI 的边界在哪里？
