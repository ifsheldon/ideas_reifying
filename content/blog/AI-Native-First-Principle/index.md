...
我们在别的地方也看到模型即产品这种模式涌现出来。一个非常突出的例子是各种定制化 SD 模型和 SD 的 LoRA 模型。很多人在 [CivitAI](https://civitai.com) 或者 [Liblib](https://liblib.art) 上发布他们训练的模型。有的是免费分享，有的是要付费授权的。
...
> 2024.10.09 更新：
> 
> 另一个例子是 [ChatGPT Canvas](https://openai.com/index/introducing-canvas/)。
> 在发布声明里，OpenAI 说到，为了让 GPT4o 能够智能地判断什么时候需要打开 Canvas，并且更好地应答 Canvas 工具提出的需求（例如编辑或重写），他们甚至蒸馏了 o1 模型的数据。
> 
> 也就是说，为了更好地实现 Canvas 这个功能，他们不仅仅写了很多工程代码，甚至还微调了 GPT4o 模型本身。
...
它只能听懂人的语音，理解不了别的声音，也就意味着在输入信息量方面，Rabbit R1 和 ChatGPT 手机 App 是一样的。当 ChatGPT 手机 App 的模型换成了能够理解声音的新一代 GPT，在各种基于声音的新用例涌现，Rabbit R1 在理解声音这方面就变成了功能机，没人会在意 Rabbit R1 是否比手机更加轻巧便捷。
...
当然，别忘了，我们还可以用文字 API 在 LLM 的上下游拓展能力。例如，检索增强生成（Retrieval Augmented Generation - RAG）在 LLM 的上游使用搜索引擎增强 LLM 的能力；而函数调用（Function Calling）则依赖 LLM 输出格式化的文本，调用下游组件来增强能力。这些都是在文本的层面来增强整个 AI OS 系统的能力。
...
> 在 [Octopus V2](https://arxiv.org/abs/2404.01744) 里，作者提出了函数词元（functional token）这个概念，其实是分词器 API 的一个特例。
> 
> 有意思的是，他们上传论文的时间，比我写完这一个章节的时间晚了一个月。
...
* AI 第一性原理对管线、组织的启示，详见 [Minimax 伙伴日更新](#minimax-linkday)：
  * 数据管线要保证信息、反馈的高效传递
  * 团队能力要保证信息、反馈的高效传导、及时响应、迭代
* AI OS 是什么样的：
...
## 2024.08.31 更新：MiniMax Link 伙伴日 {#minimax-linkday}

![minimax_linkday](./minimax_linkday.png)

今天我受邀去 MiniMax Link 伙伴日，小小地被震撼到了。

闫俊杰在演讲里讲他们的技术和用户其实在相互成就，用户的反馈会促进模型的成长。但是同时，全球只有不到 5% 的人是用户。接着讲到到他们要知难而上、做线性注意力，说：如果保持能力不变，那么（推理速度）越快的模型就能够被越多的人使用；而越多人用这个模型，这个模型就会变得更好。在这种意义上，“快“就意味着“好”。

稍后的论坛里，不同硬科技领域的创始人讲到一个点的时候都有共鸣，就是：技术重要，但是同时企业组织的形态也很重要。新的科技需要有新的企业组织形态。

这让我开始思考，可能 AI 第一性原理需要拓展到更广的层面上去讨论。例如 AI 模型和数据本身应该遵循上述的第一性原理，但是模型和数据的管线也有第一性原理，开发模型的团队、公司也应该围绕着这个第一性原理来设计。举个例子，同样是“信息量”这三个字，在收集数据的管线里的体现可能就是用户反馈的信息的质量、数量和流转速度；
而对于团队来说，围绕“信息量”需要建立的能力可能是超强的用户运营能力和迭代能力。

最后感叹一句，效果比肩传统注意力的线性注意力在伙伴日发布，可能会是人类科技的一个看似寻常但是非常重要的转折点。可能不久我们就有数以亿计的有效上下文，到那个时候，科学和文明都会在 AI 的帮助下迎来一个新的爆炸💥时代。
...
2024.05.09：添加了 Octopus V2 的引用

2024.07.16：Fixed typo

2024.08.31：添加了 MiniMax Link 伙伴日的内容

2024.10.09：添加了 ChatGPT Canvas 的内容